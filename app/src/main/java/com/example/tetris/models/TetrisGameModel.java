...
import com.example.tetris.presenters.CompletableObserver;
import com.example.tetris.presenters.GameModel;
import com.example.tetris.presenters.GameTurn;
import com.example.tetris.presenters.Observer;
import com.example.tetris.presenters.Point;
import com.example.tetris.presenters.PointType;
...
public class TetrisGameModel implements GameModel {
    ...
    private static final int GAME_SIZE2 = 34;
    private static final int PLAYING_AREA_WIDTH = 15;
    private static final int PLAYING_AREA_HEIGHT = GAME_SIZE2;
    ...
    private CompletableObserver mGameOverObserver;
    private Observer<Integer> mScoreUpdatedObserver;

    private final Handler mHandler = new Handler();

    public List<ScoreboardModel> lScores;
    ...
    @Override
    public void init() {
        mPoints = new Point[GAME_SIZE2][GAME_SIZE];
        for (int i = 0; i < GAME_SIZE2; i++) {
    ...
    private void generateUpcomingBrick() {
        ...
        switch (upcomingBrick) {
            case L:
                ...
                mUpcomingPoints[1][1].color = color;
                mUpcomingPoints[2][1].color = color;
                mUpcomingPoints[3][1].color = color;
                mUpcomingPoints[3][2].color = color;
                break;
            case T:
                mUpcomingPoints[1][1].type = PointType.BOX;
                mUpcomingPoints[2][1].type = PointType.BOX;
                ...
                mUpcomingPoints[2][1].color = color;
                mUpcomingPoints[3][1].color = color;
                mUpcomingPoints[2][2].color = color;
                break;
            case CHAIR:
                mUpcomingPoints[1][1].type = PointType.BOX;
                mUpcomingPoints[2][1].type = PointType.BOX;
            ...
            case STICK:
                ...
                mUpcomingPoints[2][1].type = PointType.BOX;
                mUpcomingPoints[3][1].type = PointType.BOX;
                mUpcomingPoints[0][1].color = color;
                mUpcomingPoints[1][1].color = color;
                mUpcomingPoints[2][1].color = color;
                mUpcomingPoints[3][1].color = color;
                break;
            case SQUARE:
                ...
                mUpcomingPoints[2][2].type = PointType.BOX;
                mUpcomingPoints[1][1].color = color;
                mUpcomingPoints[1][2].color = color;
                mUpcomingPoints[2][1].color = color;
                mUpcomingPoints[2][2].color = color;
                break;
        }
    ...
    @Override
    public void startGame(Observer<Point[][]> onGameDrawnListener) {
    ...
    private synchronized void next() {
        ...
        if (isNextMerged()) {
            ...
            while (y >= 0) {
                ...
                if (isScored) {
                    mScore++;
                    lScores = new ArrayList<>(10);
                    lScores.add(0, new ScoreboardModel("player 1", mScore));
                    ...
                    for (int i = 0; i <= y; i++) {
                        for (int j = 0; j < PLAYING_AREA_WIDTH; j++) {
                            ...
                            if (point.type == PointType.BOX) {
                                ...
                                if (i != y) {
                                    tmPoints.add(new Point(point.x, point.y + 1, PointType.BOX, false, color));
                                }
            ...
            for (Point fallingPoint : mFallingPoints) {
                ...
                tmPoints.add(new Point(fallingPoint.x, fallingPoint.y + 1, PointType.BOX, true, color));
            }
            mFallingPoints.clear();
            ...
            for (int i = 0; i < UPCOMING_AREA_SIZE; i++) {
                for (int j = 0; j < UPCOMING_AREA_SIZE; j++) {
                    if (mUpcomingPoints[i][j].type == PointType.BOX) {
                        mFallingPoints.add(new Point(j + 3, i - 4, PointType.BOX, true, color));
                    }
                }
            }
            generateUpcomingBrick();
        ...
        switch (turn) {
            case LEFT:
                ...
                if (canTurn) {
                    ...
                    for (Point fallingPoint : mFallingPoints) {
                        tmPoints.add(new Point(fallingPoint.x - 1, fallingPoint.y, PointType.BOX, true, color));
                        fallingPoint.type = PointType.EMPTY;
                        fallingPoint.isFallingPoint = false;
                    }
                    mFallingPoints.clear();
    ...
    public boolean rotatePoints(int x, int y, int size) {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                ...
                points[i][j] = new Point(x + size - 1 - i, y + j, point.type, point.isFallingPoint, color);
            }
            if (!canRotate) {
                break;
            }
    ...
    @Override
    public void setGameOverListener(CompletableObserver onGameOverListener) {
        mGameOverObserver = onGameOverListener;
    }

    @Override
    public void setScoreUpdatedListener(Observer<Integer> onScoreUpdatedListener) {
